...
export APP="${1:-tolerations-injector}"
...
kubectl delete secret tls-certs-${APP} --namespace=$NAMESPACE
kubectl create secret generic tls-certs-${APP} --from-file=${APP}.pem --from-file=${APP}.key --namespace=$NAMESPACE
